...
This task had us list several potential input and output variable pairings for a feedback system from how the crane was described, for example *F* as an input and *φ* as an output. We then had to explain the purpose of such a closed-loop system and how they would improve the crane's efficiency. This came down to stating that we needed a closed-loop control system because we were aiming to control *φ*, but it cannot be controlled directly, only as the result of other system features.

In addition to outlining our fundamental understanding of the control system we needed to develop, we had to prove it by making a generic block diagram of one input-output variable pair, showing a potential distuirbing signal and other typical components of a flow diagram. I selected to do *F*-*x<sub>t</sub>*.

{{< fig src="/images/controls-disturbed-flow.png" caption="My generalized block diagram for <em>F-x<sub>t</sub></em>" >}}
...
To make things easier for us as we made the transform functions, we were allowed to linearize our models by assuming that *φ* would in most cases be approximately zero, thus sin(*φ*) = *φ* and cos(*φ*) = 1. Our new model was:
...
{{< fig src="/images/controls-simulink-model.png" caption="My block diagram for <em>F-φ</em> in Simulink based on the mathematical model" >}}

## Transient Time Response

This was a short task that focused on having us simulating the transient response of Φ(s)/V<sub>t</sub>(s) to a step and ramp input, with an open loop and negative unity feedback, for a total of four responses plotted.

{{< fig src="/images/controls-phi-step.png" caption="Open-loop φ response to a step input of v<sub>t</sub>" >}}

{{< fig src="/images/controls-phi-ramp.png" caption="Open-loop φ response to a ramp input of v<sub>t</sub>" >}}

{{< fig src="/images/controls-phi-step-comp.png" caption="Negative unity φ response to a step input of v<sub>t</sub>" >}}

{{< fig src="/images/controls-phi-ramp-comp.png" caption="Negative unity φ response to a ramp input of v<sub>t</sub>" >}}
